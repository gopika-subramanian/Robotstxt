...
ROBOTSTXT
"""  + END + BLUE +
'\n' + '{}Onion URL Checker ({}ONIONCHECK{}){}'.format(YELLOW, RED, YELLOW, BLUE).center(67) +
'\n' + 
'\n' + 'Operation {}-> START{}'.format(YELLOW, END).center(57) + '\n')
...
def connectTor():
    ...
    if pure_ip == tor_ip:
        nowPrint("=_= tor connection failed", True)
        nowPrint("\n=_= Kill\n", True)
        os._exit(1)
    else:
...
def checkOnion(onion):
    ...
    if check_ip != pure_ip:
        ...
        nowPrint("=_= Tor connection error", True)
        nowPrint("\n=_= Kill\n", True)
        os._exit(1)
...
def readFile(file):
    try:
        ...
        nowPrint("=_= Valid input plis", True)
        nowPrint("\n=_= kill\n", True)
        os._exit(1)
...
def main():
    ...
    if len(sys.argv[1:]) > 0:
        ...
        if (len(sys.argv[1:]) == 2 and sys.argv[1] == '--output') or (len(sys.argv[1:]) == 1 and sys.argv[1] == '--active') or \
            ...
            nowPrint("\n=_= Invalid choices dont do good in life\n", False, False, True)
            os._exit(1)
        ...
        try:
            for onion in argv:
                if not onion.startswith('http') and not onion.startswith('https'):
                    nowPrint("=_= valid stuff plis", True)
                    nowPrint("\n=_= kill\n", True)
                    os._exit(1)
                else:
                    q.put(onion)
        ...
        nowPrint("Operation success <3", False, False, True)
        saved_msg = "\n[!] Results at " + str(outFile)
...
if __name__ == '__main__':
    ...
    examples = ('\nExamples:\n'+
                '  python3 robotstxt.py http://xmh57jrzrnw6insl.onion/\n'+
                '  python3 robotstxt.py -f ~/onions.txt -o ~/report.txt\n'+
                '  python3 robotstxt.py https://facebookcorewwwi.onion/ -o ~/report.txt\n')

    parser = optparse.OptionParser(epilog=examples,
                                   usage='python3 %prog {onion} [options]',
                                   prog='robotstxt.py', version=('ROBOTSTXT V.0'))

    parser.add_option('-f', '--file', action='store',
                      dest='file', help='name of onion file')

    default = 'check/robotstxt_{}'.format(str(datetime.datetime.now())[:-7].replace(' ', '_'))
